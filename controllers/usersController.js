...
module.exports.uploadPhoto = async function(socket, file) {
  try {
    ...
    .then(results => {
      socket.emit('update_complete', {type: "picture", url: data.Location, username: socket.username, time: moment(Date.now()).calendar()})
    })
...
module.exports.updateStatus = function(socket, status) {
  ...
  .then(result => {
    socket.emit('update_complete', {type: "status", status: status, username: socket.username, time: moment(Date.now()).calendar()})
  })
  .finally(() => session.close())
}